...
module.exports.login = function (req, res) {
    ...
    global.contractInstance.getSalt(1, phone, function (error, result) {
        if(!error) {
            var salt = result.toString();
            global.contractInstance.loginCustomer(phone, commonUtils.toMD5(password + salt), {from: web3.eth.coinbase}, function (error, result) {
                if (!error) {
