...
module.exports.register = function (req, res) {
    ...
    if(judgeNodeType.nodeType == 0) {
        ...
        global.contractInstance.registerManager(keys.accountAddress, req.query.phone, req.query.password, {from: web3.eth.accounts[0], gas: 1600000}, function (error, result) {
            if (!error) {
        ...
        generateAccount.generateAccounts(req.query.password, function (error, result) {
            ...
            if (result.code == 0) {
                ...
                global.contractInstance.registerManager(result.account, req.query.phone, req.query.password, {from: web3.eth.accounts[0], gas: 1600000}, function (error, result) {
                    if (!error) {
                        var eventRegisterManager = global.contractInstance.RegisterManager();
                        eventRegisterManager.watch(function (error, result) {
                            console.log("状态码：" + result.args.statusCode + "消息：" + result.args.message);
